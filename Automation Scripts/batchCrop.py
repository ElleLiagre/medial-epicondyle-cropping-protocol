...
#        the Medial Epicondyle of the Humerus - Python Script for Batch Cropping
# 
...
# Date: 2025-03-24
# 
# Description: Batch function to crop all 3D models in a specified folder. If 
# no folder path is provided, a window will prompt you to select one. Cropped 
# models will be saved in a new folder within the original folder. Note: This 
# function is for right humeri only due to orientation requirements. If run 
# for a left-sided model, the humeral diaphysis will be extracted instead
# of the medial epicondyle. More information can be found on: 
...
def crop(modelNode: vtk.vtkPolyData) -> None:
    if modelNode:
        ...
        def automaticLandmarkPlacement():
            ...
            for i in range(points.GetNumberOfPoints()):
                ...
                # Update max values for x, y, and z
                max_x = max(max_x, vertex[0])
                max_y = max(max_y, vertex[1])
                max_z = max(max_z, vertex[2])
        
                # Update min values for x, y, and z
            ...
            # Iterate through the points in the model to get Max Y
            ...
            # Iterate through the points in the model to get Min Y
            total_points_checked = 0
            for i in range(polyData.GetNumberOfPoints()):
                point = polyData.GetPoint(i)
                x, y, z = point
        ...
        # Check if the markup node exists
        if not pointListNode:
            logger.error("Point list node not found.")
        else:
        ...
        try:
            # Check for valid point list node
...
def save_model(modelNode: vtk.vtkPolyData, outputModelName: str) -> None:
    if modelNode:
        try:
            ...
            logging.info(f'Successfully exported cropped model as: {outputModelName}')
        except Exception as e:
            logger.error('Failed to export model as PLY: %s', str(e))
    else:
        logger.warning("Model node 'model' not found.")

def save_pointlist(pointListNode: vtk.vtkPolyData, outputListName: str) -> None:
    if pointListNode:
        try:
            slicer.util.saveNode(pointListNode, outputListName)
            logging.info(f'Successfully exported cropped model as: {outputListName}')
        except Exception as e:
            logger.error('Failed to export model as PLY: %s', str(e))
    else:
...
def batchCrop(folderPath: Optional[str] = None, 
              valid_extensions: List[str] = ['.obj', '.ply', '.stl'], 
              output_format: str = 'ply') -> None:
    """
    Batch process 3D models in a folder to apply cropping.
    Cropped models and log files are saved in a new folder named 'OriginalFolderName_Cropped'.

    Parameters:
        folderPath (str): Path to the folder containing models. If None, a folder dialog opens.
        valid_extensions (list): List of valid file extensions to process.
    """

    # Prompt for folder if not provided
    if not folderPath:
        folder_dialog = qt.QFileDialog()
    ...
    # Create a new folder named "OriginalFolderName_Cropped" inside the selected folder
    originalFolderName = os.path.basename(folderPath)
    croppedFolder = os.path.join(folderPath, f'{originalFolderName}_Cropped')
    os.makedirs(croppedFolder, exist_ok=True)
    logging.info(f"Created cropped folder: {croppedFolder}")

    # Add file logging in the new folder
    log_file_path = os.path.join(croppedFolder, "batch_crop.log")
    ...
    for inputFilename in os.listdir(folderPath):
        ...
        try:
            ...
            if user_input.lower() == 'yes':
                ...
                modelNode = slicer.util.getNode("finalCrop")
                outputModelName = os.path.join(croppedFolder, f"{fn}_cropped.{output_format}")
                outputModelName = handle_filename_conflict(outputModelName)
                save_model(modelNode, outputModelName)

                pointListNode = slicer.util.getNode("LM")
                outputListName = os.path.join(croppedFolder, f"{fn}_cropped.fcsv")
                outputListName = handle_filename_conflict(outputListName)
                save_pointlist(pointListNode, outputListName)

            else:
                # Get the full model and an alternative crop model nodes
                fullmodelNode = slicer.util.getNode("model")
                modelNode = slicer.util.getNode("finalCrop_2")
